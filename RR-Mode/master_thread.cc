...
bool CMasterThread::InitMasterThread()
{
	...
	// libevent库初始化，分配并且返回一个新的具有默认设置的event_base
    ...
    // 尝试创建一个用于event_base的事件,返回event指针
...
{
    // 系统调用socket()只返回一个套接口描述符（int），如果出错，则返回-1
    // evutil_socket_t类型为整型指针，可以处理socket()或者accept()的输出，而没有指针截断的风险
	...
	bzero(&servaddr, sizeof(servaddr)); // 将内存块（字符串）的前n个字节清零
